...
int read_and_parse_input(char *argv[]) {
    static char input[MAX_INPUT];  // static so argv points to it safely
    printf("sword-shell> ");
    fflush(stdout);
    ...
    char result[MAX_INPUT] = "";  // Used for caesar text
...
handle_caesar(int argc, char *argv[]) {
    if (argc < 3) {
